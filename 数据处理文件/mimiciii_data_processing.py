...
def get_connection():
    global conn
    if conn is None:
        conn = psycopg2.connect(database="mimic", user="postgres", password="123456", host="127.0.0.1", port='5433')
    ...
    for row in cur:
        if row[0] is not None:
            if type(row[0]) == float and np.isnan(row[0]):
                print("if type(row[0]) == float and np.isnan(row[0]):")
                continue
            if top_n_labels is not None and reformat(row[0], is_diagnosis, FULL) not in top_n_labels:
                print(" top_n_labels is not None and reformat(row[0], is_diagnosis, FULL) not in top_n_labels")
                continue
...
if __name__ == "__main__":
    ...
    top_n_labels = read_admission_ids(
        ...
        outdir="../../data/mimicdata/mimic3/full6/")

    # labelall=set()
    file_path = '../../data/mimicdata/mimic3/full6/label_all.txt'

    # 打开文件并写入set中的数据
    with open(file_path, 'w') as file:
        for item in label_count_dict:
            file.write(str(item) + '\n')
    # print(top_n_labels)
    ...
    read_admission_ids(
        ...
        outdir="../../data/mimicdata/mimic3/50_6/",
    ...
    for i in range(n):
        output.append(sorted_labels[i][0])
    file_path2 = '../../data/mimicdata/mimic3/50_6/label_50.txt'
