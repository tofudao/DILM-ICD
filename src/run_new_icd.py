...
def main():
    ...
    raw_datasets = load_data(args.data_new_dir)
    ...
    for epoch in tqdm(range(args.n_epoch)):
        ...
        print(" ")
        metrics_max_2, best_thre_2 = ans_test(test_dataloader, model, args.code_50)
        ##########################################
        if metrics_max is None:
            ...
            if metrics_max['f1_micro'] < metrics_max_2['f1_micro']:
                metrics_max = metrics_max_2
                best_thre = best_thre_2
    print("最好的阈值", best_thre, ", ".join(f"{k}: {v:.4f}" for k, v in metrics_max.items()))
    print()
