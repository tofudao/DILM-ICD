...
class AttentionLayer(nn.Module):
    ...
    def __init__(self, args, vocab):
        ...
        # self.first_linears = nn.ModuleList([nn.Linear(self.size, self.d_a, bias=False)])
        # self.second_linears = nn.ModuleList([nn.Linear(self.d_a, self.n_labels, bias=False)])
        # self.third_linears = nn.ModuleList([nn.Linear(self.size,self.n_labels, bias=True)])
        ...
        # print("self.third_linears",self.third_linears) #Linear(in_features=512, out_features=50, bias=True)
        # =======================================================
        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()
        ...
        # self.transformer_layer=TransformerLayers_MoE(feaSize=self.size, dk=args.dk, multiNum=args.multiNum,num_experts=4,dropout=0.1)
    ...
    def forward(self, x , label_batch=None, max_pool=None):
        ...
        if self.attention_mode == "text_label":
            ...
            for i in range(self.layer):
                ...
                fea = fea1.detach_()
                # =================================================================================
                fea = torch.cat((max_pool, fea), 2)  # =>  (batchSize × 1 × (filterSize+classNum))/
                #             print("fea",fea)
