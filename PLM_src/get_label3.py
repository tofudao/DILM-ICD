...
def main():
    ...
    if args.use_finetune:
        check = torch.load(args.best_model_path + "check.pth", map_location="cuda:0")
        print()
    ...
    for j in range(iter_label):
        # ==================================================
        print("第j轮", j)
        label_vec = get_label_vec(model, train_dataloader, eval_dataloader, test_dataloader, short_lists, j)
        # ===========================================================================
        ...
        # ===========================================================================
        positive_counts = torch.sum(train_target_tensor, axis=0)
        negative_counts = torch.sum(1 - train_target_tensor, axis=0)
        pos_weight = negative_counts/positive_counts
        # total_samples = train_target_tensor.shape[0]
        # positive_ratios = positive_counts / total_samples
        # negative_ratios = negative_counts / total_samples
        # ===========================================================================
        del label_vec
        ...
        for k in range(len(short_lists[j])):
            ...
            criterions=nn.BCEWithLogitsLoss(pos_weight=pos_weight[k])
            # ==================================================================
            all_preds_raw, all_preds, all_labels = get_test_semble(model_semble, test_semble_dataloader)
            ...
            print(" ")
            # ==================================================================
            # all_preds_raw, all_preds, all_labels = None, None, None
            # metrics = None
            del all_preds_raw, all_preds, all_labels,metrics
            gc.collect()
            # ================================================================
            ...
            batch_id = 0
            # ================================================================
            all_preds_raw, all_preds, all_labels = get_test_semble(model_semble, eval_semble_dataloader)
            # model_semble.eval()
            # all_preds = []
            # all_preds_raw = []
            # all_labels = []
            ...
            print(datetime.datetime.now().strftime('%H:%M:%S'))
            # =====================================================================
            del metrics_max, metrics_max_val, metrics_max_auc,checkpoint
            gc.collect()
            # metrics_max = None
            # metrics_max_val = None
            # metrics_max_auc = None
            # checkpoint = None
            # =========================================================================
            checkpoint = torch.load(args.best_model_path + 'sembel/check_{}.pth'.format(i), map_location="cuda:0")
            ...
            # # ===========================================================================
            # dataset = None
            # train_semble_dataloader=None
            # eval_semble_dataloader=None
            # test_semble_dataloader=None

            lr_scheduler = None
            optimizer = None
            losses = []
            del dataset,train_semble_dataloader,eval_semble_dataloader,test_semble_dataloader
            del checkpoint, model_semble,  all_preds_raw, all_preds, all_labels,lr_scheduler,optimizer,losses
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            gc.collect()
        # =======================================
        # train_input_tensor = None
        # train_target_tensor = None
        # val_input_tensor = None
        # val_target_tensor = None
        # test_input_tensor = None
        # test_target_tensor = None
        del train_input_tensor,train_target_tensor,val_input_tensor,val_target_tensor,test_input_tensor,test_target_tensor
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        gc.collect()
    # # =========================================================================
    ...
    # print(
    #     f"测试集开始时: F1_micro: {metrics['f1_micro']:.4f}, F1_macro: {metrics['f1_macro']:.4f}, auc_micro: {metrics['auc_micro']:.4f}, auc_macro: {metrics['auc_macro']:.4f}, prec_at_8: {metrics['prec_at_8']:.4f}")
    # print(" ")
    # # ========================================================================
    ...
    # print(
    #     f"测试集结束后: F1_micro: {metrics['f1_micro']:.4f}, F1_macro: {metrics['f1_macro']:.4f}, auc_micro: {metrics['auc_micro']:.4f}, auc_macro: {metrics['auc_macro']:.4f}, prec_at_8: {metrics['prec_at_8']:.4f}")
    # print(" ")
    # =============================================================================
    att_third_linear={}
    ...
    att_third_linear = torch.load(args.best_model_path + 'third_linear.pth', map_location="cuda:0")
